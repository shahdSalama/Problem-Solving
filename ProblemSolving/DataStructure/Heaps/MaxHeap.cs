...
namespace HackerRank.DataStructure
{
    public class MaxHeap
    {
        ...
        /// <summary>
        /// look at the max element , which is the root node
        /// </summary>
        /// <returns></returns>
        public int peak()
        {
        ...
        /// <summary>
        /// remove the max element which is the root node
        /// </summary>
        public int poll()
        {
            if (size == 0) throw new InvalidOperationException();
        ...
        public void heapifyDown()
        {
            ...
            while (hasLeftChild(index))
            {
                ...
                if (hasRightChild(index) && GetRightChild(index) > maxValue)
                {
                    maxValue = GetRightChild(index);
                    maxValueIndex = getRightChildIndex(index);
                }
                if (items[index] > maxValue) break;
                else
                {
